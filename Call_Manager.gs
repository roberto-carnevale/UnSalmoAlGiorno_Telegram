 ...
 // Make sure to only reply to json requests
  if(e.postData.type == "application/json") {
    ...
    bus.on(/\/start/, function () {
      if (bot.update.message.from.id == bot.update.message.chat.id) {
        ...
        this.replyToSender("  ti arriverà un versetto al giorno per Lodi. Col comando /change puo scegliere di ricevere Compieta o entrambi i tempi del Salterio.");
        this.replyToSender("Per sospendere o riprendere l'invio automatico usa i comandi /suspend e /resume");
        this.replyToSender("Per disiscriverti puoi usare il comando /stop");
        ...
        MailApp.sendEmail("kn35roby@gmail.com", "New Subscription" , JSON.stringify(bot.update));
      } else {
    ...
    bus.on(/\/suspend/, function () {
      ...
      if (line_id > -1) { 
        ...
        bot.pushMessage("Sospendo l'invio automatico di un Salmo al giorno", parseInt(bot.update.message.chat.id));
      }
  ...
  //########TASTIERA###### 
    ...
    bus.trays_request(/#Lodi/, function () {
      ...
      bot.replyToSender("Un versetto per Lodi, per cominciare bene la giornata!");
      bot.destroyKeyboard();
    ...
    bus.trays_request(/#Compieta/, function () {
      ...
      bot.replyToSender("Un versetto della Compieta del giorno per una buona notte!");
      bot.destroyKeyboard();
